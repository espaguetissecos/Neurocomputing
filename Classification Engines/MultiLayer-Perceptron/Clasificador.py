...
class ClasificadorPerceptronMulticapa(Clasificador):
    ...
    def entrenamiento(self, datos, f_nclases, salida=None, normalizar=True,atributosDiscretos=None, diccionario=None):
        ...
        for k in range(0, self.n_epocas):
            ...
            for i in range(0, nrows):
                ...
                ##
                for l in range (0, self.num_neuronas_salida):
                    self.vector_delta_y[l] = (datostrain[i][ncols - nclases + l] - self.vector_yout[l])*self.derivada_sigmoide_bipolar(self.vector_yin[l])

                    for j in range(0, self.num_neuronas_oculta):
                        self.vector_incremento_pesos_y[l][j] = self.tasa_aprendizaje * self.vector_delta_y[l] * self.vector_zout[j]
                ...
                for l in range(0, self.num_neuronas_oculta):
                    ...
                    self.vector_delta_z[l] = self.vector_delta_zin[l] * self.derivada_sigmoide_bipolar(self.vector_zin[l])

                    for j in range(0, ncols - nclases):
                        self.vector_incremento_pesos_z[l][j] = self.tasa_aprendizaje * self.vector_delta_z[l] * datostrain[i][j]
